...
export class Lexer {
  ...
  constructor(source: string) {
    ...
    // if the source is empty (the only condition at this point where the length and the currentIndex will be the same)
  ...
  public nextToken(): Token {
    ...
    switch (this.currentChar) {
      case ":": {
        ...
        // isAlpha -> is alphanumeric
        if (!this.isAlpha(nextCharacter)) {
          throw new Error(`Invalid ":" at line ${this.lineNumber}`);
        } else {
          // need to return this immediately so it dosen't accidentally skip over/consume another character
      ...
      default:
        if (this.isAlpha(this.currentChar)) {
          // read identifier
          // also determine if it is a statement
  ...
  // might move these methods out of here
